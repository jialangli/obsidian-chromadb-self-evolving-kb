...
def eval_gate(baseline_rt, candidate_rt, cases: list = None) -> dict:
    ...
    tol = getattr(cfg, "EVAL_REGRESSION_TOLERANCE", 0.0)
    floor_ok = cand["hit5"] >= cfg.EVAL_MIN_HIT5_FLOOR
    quality_ok = cand["hit5"] >= cfg.EVAL_MIN_HIT5_PROMOTE_FLOOR
    mrr_ok = cand["mrr"] >= base["mrr"] - tol - eps
    verdict = bool(floor_ok and quality_ok and mrr_ok)

    reasons.append(
        f"绝对质量下限 Hit@5>={cfg.EVAL_MIN_HIT5_FLOOR:.0%}：候选 {cand['hit5']:.0%} "
        f"{'达标' if floor_ok else '未达标'}"
    )
    reasons.append(
        f"高绝对质量地板 Hit@5>={cfg.EVAL_MIN_HIT5_PROMOTE_FLOOR:.0%}：候选 {cand['hit5']:.0%} "
...
if __name__ == "__main__":
    ...
    print("== 离线评估闸门（自比，应达标）==")
    print(
        f"基线  Hit@1={res['baseline']['hit1']:.0%} Hit@5={res['baseline']['hit5']:.0%} MRR={res['baseline']['mrr']:.3f}"
    )
    print(
        f"候选  Hit@1={res['candidate']['hit1']:.0%} Hit@5={res['candidate']['hit5']:.0%} MRR={res['candidate']['mrr']:.3f}"
    )
