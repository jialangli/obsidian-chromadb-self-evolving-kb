...
def run(apply: bool = False, force: bool = False, min_positive: int = None,
        epochs: int = 1, lr: float = 2e-5, batch_size: int = 16,
        scale: float = 1.0) -> dict:
    min_positive = min_positive or C.LOOP_MIN_POSITIVE_FEEDBACK
    st = C.load_active()
    ...
    if not force and len(positive) < min_positive:
        msg = (f"正例反馈不足（{len(positive)} < {min_positive}），暂不训练。"
               f"可 --force 强制演示，或等业务积累更多采纳反馈。")
        print("[LOOP]", msg)
        C.log_run({"event": "skip_insufficient_feedback", "positive": len(positive),
    ...
    # ── 3. 对比学习微调 ──
    version = C.next_model_version()
    checkpoint = str(C.MODELS_DIR / f"bge_ft_v{version}")
    ft = fine_tune_bge.train(triples, checkpoint, base_model=st["active_bge_model"],
                             epochs=epochs, lr=lr, batch_size=batch_size, scale=scale)
    print(f"[LOOP] 微调完成: {ft['out_dir']}")

    # ── 4. 构建候选集合（与 base 隔离）──
    cand_coll = C.candidate_collection_name(version)
    ...
    if not info.get("built"):
        ...
        C.log_run({"event": "candidate_build_failed", "version": version})
    ...
    if gate["verdict"]:
        ...
        if apply:
            st["ab"] = {
                ...
                "traffic_ratio": C.AB_TRAFFIC_RATIO,
                "started_at": _now(),
                "baseline_metrics": gate["baseline"],
            }
            C.save_active(st)
            closed_loop_runtime.reset_hubs()
            print(f"[LOOP] ✅ 已晋升灰度 A/B：候选={cand_coll}，"
                  f"流量={int(C.AB_TRAFFIC_RATIO*100)}%（重启 MCP/API 后生效）")
        else:
            print(f"[LOOP] [dry-run] 将晋升灰度 A/B：候选={cand_coll}"
                  f"（加 --apply 真正写入 active_model.json）")
    else:
        action = "rollback_offline"
        print(f"[LOOP] ⚠️ 候选未达标 → 自动回滚：保持基线 {st['active_bge_collection']}，"
    ...
    record = {
        "event": "closed_loop_run", "version": version, "action": action,
        "n_triples": len(triples), "triple_source": src,
        "candidate_collection": cand_coll,
        "gate_verdict": gate["verdict"], "gate_deltas": gate["deltas"],
        "applied": apply,
    }
    C.log_run(record)
    return {"action": action, "gate": gate, "version": version,
            "candidate_collection": cand_coll}
...
if __name__ == "__main__":
    ...
    ap.add_argument("--scale", type=float, default=1.0, help="余弦相似度缩放（MNRL 温度倒数，默认 1.0 原始余弦）")
    ap.add_argument("--batch-size", type=int, default=16, help="微调批大小（越大 in-batch 负例越多）")
    args = ap.parse_args()
    run(apply=args.apply, force=args.force, min_positive=args.min_positive,
        epochs=args.epochs, lr=args.lr, batch_size=args.batch_size, scale=args.scale)
